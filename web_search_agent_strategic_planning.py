"""
Web Search Agent for Strategic Planning & Business Development Domain
"""
import logging
import os
import requests
...
class StrategicPlanningWebSearchAgent:
    """Web search agent specialized in Strategic Planning & Business Development domain"""
    
    def __init__(self):
        self.domain_name = "Strategic Planning & Business Development"
        self.domain_key = "strategic_planning"
    ...
    def search(self, problem_statement: str, query: Optional[str] = None) -> List[Dict]:
        """
        Search the web for strategic planning solutions related to the problem
        ...
        try:
            ...
            logger.info(f"Strategic Planning Agent: Searching for '{query}'")
            ...
            # Filter and rank results for strategic planning relevance
            filtered_results = self._filter_strategic_planning_results(results, problem_statement)
            
            # Return top 5 results
            return filtered_results[:5]
            
        except Exception as e:
            logger.error(f"Error in Strategic Planning web search: {str(e)}")
            return self._get_fallback_results(problem_statement)
    
    def _generate_search_query(self, problem_statement: str) -> str:
        """Generate a focused search query for strategic planning domain"""
        # Extract key terms from problem statement
        problem_lower = problem_statement.lower()
        
        # Strategic planning specific keywords
        keywords = []
        if any(term in problem_lower for term in ["strategy", "strategic", "planning", "growth"]):
            keywords.append("strategic planning")
        if any(term in problem_lower for term in ["business development", "expansion", "growth"]):
            keywords.append("business development")
        if any(term in problem_lower for term in ["roadmap", "vision", "goals"]):
            keywords.append("strategic roadmap")
        if any(term in problem_lower for term in ["competitive", "advantage", "positioning"]):
            keywords.append("competitive strategy")
        
        # Build query
        base_query = "strategic planning solutions"
        if keywords:
            query = f"{' '.join(keywords[:2])} {base_query} business problem"
        else:
            query = f"{problem_statement[:100]} strategic planning solutions"
    ...
    def _filter_strategic_planning_results(self, results: List[Dict], problem_statement: str) -> List[Dict]:
        """Filter and rank results for strategic planning relevance"""
        # Keywords that indicate strategic planning relevance
        relevant_keywords = [
            "strategic planning", "business strategy", "strategic management", "business development",
            "growth strategy", "strategic roadmap", "competitive strategy", "strategic framework"
        ]
    ...
    def _get_fallback_results(self, problem_statement: str) -> List[Dict]:
        ...
        return [
            {
                "title": "Strategic Planning & Business Development Solutions",
                "url": "",
                "content": f"Based on your problem: {problem_statement[:200]}... Strategic planning solutions typically involve developing long-term vision, setting strategic goals, creating roadmaps, competitive positioning, and business model innovation to drive sustainable growth.",
                "score": 0.5
            }
        ]
    ...
    def format_results_for_chat(self, results: List[Dict]) -> str:
        ...
        if not results:
            return "<p>No search results found for Strategic Planning & Business Development domain.</p>"
        
        html = f"""
        <h3>🔍 Strategic Planning & Business Development - Top 5 Solutions</h3>
        <p>Here are the best solutions and insights I found for your strategic planning challenges:</p>
