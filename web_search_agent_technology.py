"""
Web Search Agent for Technology & Digital Transformation Domain
"""
import logging
import os
import requests
...
class TechnologyWebSearchAgent:
    """Web search agent specialized in Technology & Digital Transformation domain"""
    
    def __init__(self):
        self.domain_name = "Technology & Digital Transformation"
        self.domain_key = "technology"
    ...
    def search(self, problem_statement: str, query: Optional[str] = None) -> List[Dict]:
        """
        Search the web for technology and digital transformation solutions related to the problem
        ...
        try:
            ...
            logger.info(f"Technology Agent: Searching for '{query}'")
            ...
            # Filter and rank results for technology relevance
            filtered_results = self._filter_technology_results(results, problem_statement)
            
            # Return top 5 results
            return filtered_results[:5]
            
        except Exception as e:
            logger.error(f"Error in Technology web search: {str(e)}")
            return self._get_fallback_results(problem_statement)
    
    def _generate_search_query(self, problem_statement: str) -> str:
        """Generate a focused search query for technology domain"""
        # Extract key terms from problem statement
        problem_lower = problem_statement.lower()
        
        # Technology specific keywords
        keywords = []
        if any(term in problem_lower for term in ["technology", "tech", "software", "system", "digital"]):
            keywords.append("digital transformation")
        if any(term in problem_lower for term in ["automation", "automate", "efficiency"]):
            keywords.append("automation solutions")
        if any(term in problem_lower for term in ["platform", "software", "application"]):
            keywords.append("software solutions")
        if any(term in problem_lower for term in ["integration", "api", "system"]):
            keywords.append("system integration")
        
        # Build query
        base_query = "technology digital transformation solutions"
        if keywords:
            query = f"{' '.join(keywords[:2])} {base_query} business problem"
        else:
            query = f"{problem_statement[:100]} technology digital transformation solutions"
            ...
            if response.status_code == 200:
                ...
                if result.get("answer"):
                    search_results.insert(0, {
                        "title": "Technology & Digital Transformation Insights",
                        "url": "",
    ...
    def _filter_technology_results(self, results: List[Dict], problem_statement: str) -> List[Dict]:
        """Filter and rank results for technology relevance"""
        # Keywords that indicate technology relevance
        relevant_keywords = [
            "technology", "digital transformation", "automation", "software solutions",
            "system integration", "platform", "cloud", "ai", "machine learning", "digitalization"
        ]
    ...
    def _get_fallback_results(self, problem_statement: str) -> List[Dict]:
        ...
        return [
            {
                "title": "Technology & Digital Transformation Solutions",
                "url": "",
                "content": f"Based on your problem: {problem_statement[:200]}... Technology and digital transformation solutions typically involve software implementation, automation, system integration, cloud migration, AI/ML adoption, and digital platform development to modernize business operations and drive innovation.",
                "score": 0.5
            }
        ]
    ...
    def format_results_for_chat(self, results: List[Dict]) -> str:
        ...
        if not results:
            return "<p>No search results found for Technology & Digital Transformation domain.</p>"
        
        html = f"""
        <h3>🔍 Technology & Digital Transformation - Top 5 Solutions</h3>
        <p>Here are the best solutions and insights I found for your technology and digital transformation challenges:</p>
