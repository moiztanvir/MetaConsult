"""
Web Search Agent for Market Research & Analysis Domain
"""
import logging
import os
import requests
...
class MarketResearchWebSearchAgent:
    """Web search agent specialized in Market Research & Analysis domain"""
    
    def __init__(self):
        self.domain_name = "Market Research & Analysis"
        self.domain_key = "market_research"
    ...
    def search(self, problem_statement: str, query: Optional[str] = None) -> List[Dict]:
        """
        Search the web for market research solutions related to the problem
        ...
        try:
            ...
            logger.info(f"Market Research Agent: Searching for '{query}'")
            ...
            # Filter and rank results for market research relevance
            filtered_results = self._filter_market_research_results(results, problem_statement)
            
            # Return top 5 results
            return filtered_results[:5]
            
        except Exception as e:
            logger.error(f"Error in Market Research web search: {str(e)}")
            return self._get_fallback_results(problem_statement)
    
    def _generate_search_query(self, problem_statement: str) -> str:
        """Generate a focused search query for market research domain"""
        # Extract key terms from problem statement
        problem_lower = problem_statement.lower()
        
        # Market research specific keywords
        keywords = []
        if any(term in problem_lower for term in ["market", "customer", "competitor", "demand"]):
            keywords.append("market research")
        if any(term in problem_lower for term in ["customer", "client", "user"]):
            keywords.append("customer analysis")
        if any(term in problem_lower for term in ["competitor", "competition", "competitive"]):
            keywords.append("competitive intelligence")
        if any(term in problem_lower for term in ["survey", "feedback", "data"]):
            keywords.append("market data analysis")
        
        # Build query
        base_query = "market research solutions"
        if keywords:
            query = f"{' '.join(keywords[:2])} {base_query} business problem"
        else:
            query = f"{problem_statement[:100]} market research solutions"
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for item in result.get("results", []):
                    # Tavily already provides content summary; expand it to 200-300 words
                    content = item.get("content", "")
                    # Ensure content is at least 200-300 words; if too short, fetch full page
                    if content and len(content.split()) < 150:
                        # Content is short, try to fetch full page description
                        url = item.get("url", "")
                        if url:
                    ...
                    search_results.append({
                        ...
                        "url": item.get("url", ""),
                        "content": content,
                        "description": self._create_detailed_description(content, 250),  # 200-300 words
    ...
    def _filter_market_research_results(self, results: List[Dict], problem_statement: str) -> List[Dict]:
        """Filter and rank results for market research relevance"""
        # Keywords that indicate market research relevance
        relevant_keywords = [
            "market research", "market analysis", "customer research", "competitive analysis",
            "market intelligence", "consumer insights", "market data", "survey", "market trends"
        ]
    ...
    def _get_fallback_results(self, problem_statement: str) -> List[Dict]:
        ...
        return [
            {
                "title": "Market Research & Analysis Solutions",
                "url": "",
                "content": f"Based on your problem: {problem_statement[:200]}... Market research solutions typically involve customer surveys, competitive analysis, market segmentation, and data-driven insights to understand market dynamics and customer needs.",
                "score": 0.5
            }
        ]
    ...
    def format_results_for_chat(self, results: List[Dict]) -> str:
        ...
        if not results:
            return "<p>No search results found for Market Research & Analysis domain.</p>"
        
        html = f"""
        <h3>🔍 Market Research & Analysis - Top 5 Solutions</h3>
        <p>Here are the best solutions and insights I found for your market research challenges:</p>
        ...
        for i, result in enumerate(results[:5], 1):
            ...
            if description and not description.endswith('.'):
                description += '.'
            # Limit to ~200 characters for 2-3 lines
